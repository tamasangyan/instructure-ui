...
| Pagination | labelNumberInput             | `(totalPageNumber: number) => React.ReactNode`               | No       | `(numberOfPages: number) => `of ${numberOfPages}`` | Label for number input (**only** for `input` variant)                                                                  |
...
| Pagination | inputRef | `(inputElement: HTMLInputElement \| null) => void` | No | - | provides a reference to the html input element (**only** for `input` variant) |
